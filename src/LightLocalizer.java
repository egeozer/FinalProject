...
public class LightLocalizer {
	...
	public void doLocalization(Odometer odo, Navigation navi, SampleProvider colorSensor, float[] colorData, double lightSensorDist) {
		this.odo = odo;
		this.navi = navi;
		this.colorSensor = colorSensor;
		this.colorData = colorData;
		
		// allows access to the motors
		EV3LargeRegulatedMotor[] motors = this.odo.getMotors();
		this.leftMotor = motors[0];
		this.rightMotor = motors[1];
		
		// constants
		this.lightSensorDist = lightSensorDist;			// distance added to compensate the distance from the center and to the lightSensor
		int travelSpeed = 100;							// travel speed for the robot during localization
